...
import ProofForm from "@/components/forms/ProofForm";
...
const Page = () => {
  ...
  return (
    <Tabs defaultValue="activity" className="w-full h-screen md:px-40 mx-auto">
              ...
              {notifications.map((notification) => (
                ...
                >
                  ...
                  <span className="text-[13px] text-right">
                    {convertToRelativeTime(notification.$createdAt)}
                  </span>
                </Link>
              ))}
            </div>
      ...
      <TabsContent value="matches" className="mt-14 sm:mt-0">
        {/* Matches Section */}
              ...
              {matches.map((match: any) => (
                ...
                >
                  <div className="h-8 w-8 md:h-14 md:w-14 rounded-full overflow-hidden">
                    <Image
                      src={match.senderDetails?.profilePhotoUrl || "/default-avatar.png"}
                  ...
                  <span className="text-[13px] text-right">
                    {convertToRelativeTime(match.$createdAt)}
                  </span>
                  ...
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      {match.status === "pending" ? (
                        ...
                        <Button className="text-white font-semibold text-[14px] sm:base bg-brand hover:bg-brand-100">
                          Get Coins
                        </Button>
                      )}
                    </AlertDialogTrigger>
                    <AlertDialogContent className="rounded-md">
                      <AlertDialogHeader>
                        <AlertDialogTitle className="flex font-bold justify-between items-center">
                      ...
                      {match.status === "confirmed" && (
                        <div>
                          <h1 className="font-bold">
                            Submit Photo of both of you to claim coins
                          </h1>
                          <ProofForm accountId={user.accountId} />
                        </div>
                      )}
                    </AlertDialogContent>
                  </AlertDialog>
